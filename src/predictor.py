...
class Predictor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Ensure the input has a batch dimension: [N, C, H, W]
        if len(x.shape) == 3:
            x = x.unsqueeze(0)

        # 1. Resize the input to 256x256 (equivalent to transforms.Resize(256))
        x = F.interpolate(x, size=(256, 256), mode='bilinear', align_corners=False)

        # 2. Center crop to 224x224 (equivalent to transforms.CenterCrop(224))
        start = (x.shape[2] - 224) // 2
        end = start + 224
        x = x[:, :, start:end, start:end]

        # 3. Normalize the input using the provided mean and std
        x = (x - self.mean.view(1, 3, 1, 1)) / self.std.view(1, 3, 1, 1)

        # 4. Pass through the model to get logits
        x = self.model(x)
