...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    # 1. Get the number of features extracted by the backbone (e.g., resnet18)
    num_ftrs = model_transfer.fc.in_features  # This gives the input size of the final layer
    
    # 2. Create a new linear layer with the appropriate number of inputs and outputs
    model_transfer.fc = nn.Linear(num_ftrs, n_classes)  # Output should match n_classes
